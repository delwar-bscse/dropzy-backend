...
function createModule(name) {
    ...
    const folderPath = path_1.default.join(__dirname, 'app', 'modules', folderName);
 ...
 */
function updateRouterFile(folderName, camelCaseName) {
    const routerPath = path_1.default.join(__dirname, 'app/routes', 'index.ts');
    ...
    const apiRoutesRegex = /export const apiRoutes: \{ path: string; route: any \}\[] = \[(.*?)\]/s;
    ...
    if (match) {
        ...
        if (!currentRoutes.includes(routeEntry)) {
            const updatedRoutes = currentRoutes
                ? `${currentRoutes}\n  ${routeEntry}`
                : `${routeEntry}`;
            routerFileContent = routerFileContent.replace(apiRoutesRegex, `export const apiRoutes: { path: string; route: any }[] = [\n  ${updatedRoutes}\n]`);
        }
    }
    else {
        console.error('Failed to find apiRoutes array. Ensure the app.ts file has a properly defined apiRoutes array.');
        return;
    }
