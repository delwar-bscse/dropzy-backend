...
const loginUserFromDB = async (payload) => {
    ...
    const isExistUser = await user_model_1.UserModel.findOne({ email }).select('+password');
    if (!isExistUser) {
        throw new ApiErrors_1.default(http_status_codes_1.StatusCodes.BAD_REQUEST, "User doesn't exist!");
    }
    if (isExistUser.isDeleted) {
        throw new ApiErrors_1.default(http_status_codes_1.StatusCodes.BAD_REQUEST, 'This user is deleted! Please contact to admin');
    }
    ...
    if (password && !(await user_model_1.UserModel.isMatchPassword(password, isExistUser.password))) {
        throw new ApiErrors_1.default(http_status_codes_1.StatusCodes.BAD_REQUEST, 'Password is incorrect!');
    }
    if (role) {
        const res = await user_model_1.UserModel.findOneAndUpdate({ email }, { role }, { new: true });
        if (res) {
...
const forgetPasswordToDB = async (email) => {
    const isExistUser = await user_model_1.UserModel.isExistUserByEmail(email);
    if (!isExistUser) {
        throw new ApiErrors_1.default(http_status_codes_1.StatusCodes.BAD_REQUEST, "User doesn't exist!");
    }
    //send mail
    ...
    await user_model_1.UserModel.findOneAndUpdate({ email }, { $set: { authentication } });
};
//verify email
const verifyEmailToDB = async (payload) => {
    var _a, _b;
    const { email, oneTimeCode } = payload;
    const isExistUser = await user_model_1.UserModel.findOne({ email }).select('+authentication');
    if (!isExistUser) {
        throw new ApiErrors_1.default(http_status_codes_1.StatusCodes.BAD_REQUEST, "User doesn't exist!");
    }
    if (!oneTimeCode) {
    ...
    let data;
    if (!isExistUser.verified) {
        await user_model_1.UserModel.findOneAndUpdate({ _id: isExistUser._id }, { verified: true, authentication: { oneTimeCode: null, expireAt: null } });
        message = 'Email verify successfully';
    }
    else {
        await user_model_1.UserModel.findOneAndUpdate({ _id: isExistUser._id }, {
            authentication: {
...
const resetPasswordToDB = async (token, payload) => {
    ...
    const isExistUser = await user_model_1.UserModel.findById(isExistToken.user).select('+authentication');
    ...
    await user_model_1.UserModel.findOneAndUpdate({ _id: isExistToken.user }, updateData, { new: true });
};
const changePasswordToDB = async (user, payload) => {
    const { currentPassword, newPassword, confirmPassword } = payload;
    const isExistUser = await user_model_1.UserModel.findById(user.id).select('+password');
    if (!isExistUser) {
        throw new ApiErrors_1.default(http_status_codes_1.StatusCodes.BAD_REQUEST, "User doesn't exist!");
    }
    //current password match
    if (currentPassword && !(await user_model_1.UserModel.isMatchPassword(currentPassword, isExistUser.password))) {
        throw new ApiErrors_1.default(http_status_codes_1.StatusCodes.BAD_REQUEST, 'Password is incorrect');
    }
...
const newAccessTokenToUser = async (token) => {
    ...
    const isExistUser = await user_model_1.UserModel.findById(verifyUser === null || verifyUser === void 0 ? void 0 : verifyUser.id);
    if (!isExistUser) {
...
const resendVerificationEmailToDB = async (email) => {
    // Find the user by ID
    const existingUser = await user_model_1.UserModel.findOne({ email: email }).lean();
    if (!existingUser) {
    ...
    const accountEmailTemplate = emailTemplate_1.emailTemplate.resendOtp(emailValues);
    ...
    await user_model_1.UserModel.findOneAndUpdate({ email: email }, { $set: { authentication } }, { new: true });
};
const swapUserRoleFromDB = async (id) => {
    ...
    isExistUser.role = isExistUser.role === user_1.USER_ROLES.SENDER ? user_1.USER_ROLES.COURIER : user_1.USER_ROLES.SENDER;
    await isExistUser.save();
    //create token
    const accessToken = jwtHelper_1.jwtHelper.createToken({ id: isExistUser._id, role: isExistUser.role, email: isExistUser.email }, config_1.default.jwt.jwt_secret, config_1.default.jwt.jwt_expire_in);
    //create token
...
const deleteUserFromDB = async (user, password) => {
    const isExistUser = await user_model_1.UserModel.findById(user.id).select('+password').lean();
    if (!isExistUser) {
        throw new ApiErrors_1.default(http_status_codes_1.StatusCodes.BAD_REQUEST, "User doesn't exist!");
    }
    //check match password
    ...
    const updateUser = await user_model_1.UserModel.findByIdAndUpdate(user.id, { isDeleted: true }, { new: true }).lean();
    if (!updateUser) {
        throw new ApiErrors_1.default(http_status_codes_1.StatusCodes.BAD_REQUEST, "User doesn't exist!");
    }
    return;
